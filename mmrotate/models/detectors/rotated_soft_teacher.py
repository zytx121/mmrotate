...
class RotatedSoftTeacher(SoftTeacher):
    ...
    def compute_uncertainty_with_aug(
        ...
        reg_channel = max(
            [results.bboxes.shape[-1] for results in results_list]) // 5
        bboxes = [
            ...
            if results.bboxes.numel() > 0 else results.bboxes.new_zeros(
                self.semi_train_cfg.jitter_times, 0, 5 * reg_channel).float()
            for results in results_list
        ]
        bboxes = [rbox2hbox(bbox) for bbox in bboxes]
