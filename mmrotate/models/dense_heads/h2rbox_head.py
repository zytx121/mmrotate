...
from mmdet.models.utils import (filter_scores_and_topk, get_box_tensor,
...
class H2RBoxHead(RotatedFCOSHead):
    ...
    def __init__(self,
                 ...
                 loss_aug_center: ConfigType = dict(
                     type='mmdet.L1Loss', loss_weight=1.0),
                 loss_aug_shape: ConfigType = dict(
                     type='mmdet.IoULoss', loss_weight=1.0),
                 loss_aug_angle: ConfigType = dict(
                     type='mmdet.L1Loss', loss_weight=1.0),
        ...
        super().__init__(
            ...
            **kwargs)
        self.loss_aug_center = MODELS.build(loss_aug_center)
        self.loss_aug_shape = MODELS.build(loss_aug_shape)
        self.loss_aug_angle = MODELS.build(loss_aug_angle)
    ...
    def get_targets(
        ...
        num_gts = [len(gt) for gt in batch_gt_instances]
        gt_idx_list = []
        gt_idx_offset = 0
        for bid, gt_idx in enumerate(_gt_idx_list):
            gt_idx_list.append((gt_idx + gt_idx_offset).split(num_points, 0))
    ...
    def _predict_by_feat_single(self,
        ...
        if self.rect_classes:
            ...
            for id in self.rect_classes:
                ...
                results['bboxes'][inds, :-1] = hbox2rbox(
                    rbox2hbox(bboxes[inds, :-1]))
        results.bboxes = RotatedBoxes(bboxes)
